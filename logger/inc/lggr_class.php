...
LIMIT $from,$count";
	    ...
	    // function
    function getFirewall($from = 0, $count = LggrState::PAGELEN) {
        $perfSize = new LggrPerf();
        $perfData = new LggrPerf();
        
        //$v = $this->getViewName();
        
        $sqlSize = "SELECT COUNT(*) AS c FROM RoutingFW";
        $sqlData = "
SELECT * FROM RoutingFW
ORDER BY `date` DESC
LIMIT $from,$count";
	...
	//function
	function getDNSQueries($from = 0, $count = LggrState::PAGELEN) {
        $perfSize = new LggrPerf();
        $perfData = new LggrPerf();
        
        //$v = $this->getViewName();
        
        $sqlSize = "SELECT COUNT(*) AS c FROM DNSQueries";
        $sqlData = "
